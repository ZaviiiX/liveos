...
// ======== GLOBAL CONTRACT ADDRESS (pomaknuto gore da ga sve koristi) ========
...
function handleLogin() {
  ...
  if (password === CORRECT_PASSWORD) {
    ...
    // Show welcome window on login (center)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // (opcionalno) desktop ikona
  const buyIcon = document.getElementById('buyIcon'); // ako si je dodao u HTML

  if (buyIcon && buyWindow) {
  ...
  // Prikaži CA tekst u prozoru (GLOBALNI CONTRACT_ADDRESS)
  if (buyCA) buyCA.textContent = CONTRACT_ADDRESS;

  // Copy handler (koristi već postojeću funkciju copyContractAddress)
  if (buyCopyCA) {
    buyCopyCA.addEventListener('click', () => {
      copyContractAddress(buyCopyCA);
    });
  }
  ...
  if (buyWindow) {
    ...
    buyWindow.querySelector('.maximize').addEventListener('click', (e) => {
      e.stopPropagation();
      buyWindow.classList.toggle('maximized');
    });

    // Dovedi u fokus kad se klikne
    buyWindow.addEventListener('mousedown', () => {
      bringWindowToFront(buyWindow);
    });
  }
  ...
  if (twitterLink) {
    twitterLink.addEventListener('click', (e) => {
      e.preventDefault();
      const twitterUrl = 'https://x.com/Solana XP_'; // Update this URL when Twitter link is provided
  ...
  // Copy CA button handlers (postojeći gumbi u welcome/browser)
  ...
  if (myComputerIcon && myComputerWindow) {
  ...
  if (helperIcon && desktopBuddy) {
    helperIcon.addEventListener('dblclick', () => {
      ...
      setTimeout(() => {
        showBotBubble('Hello! I\'m Solana XP Bot! 👋 Click me to chat!');
      }, 800);
    });
  }

  // Close buddy
  if (buddyClose && desktopBuddy) {
  ...
  const helperResponses = {
    ...
    // Information queries
    'what is Solana XP': 'Solana XP is a retro-styled operating system interface that brings nostalgia and modern functionality together! It features a beautiful cyan gradient design and classic window aesthetics.',
    ...
    'features': 'Solana XP includes: Solana XP Browser for BNB token info, My Computer for system details, Mail with success stories, this Desktop Helper for assistance, an empty Recycle Bin, and a Power button with cool animations!',
    'who created': 'Solana XP was created as a nostalgic tribute to classic operating systems, combining retro aesthetics with modern web technologies.',
    'help': 'I can help you learn about Solana XP! Try asking: "What is Solana XP?", "How do I use this?", "What features are available?", or just say hi!',
    
    // Fun interactions
    'how are you': 'I\'m doing fantastic! 🚀 Thanks for asking! I\'m always excited to help users explore Solana XP. How are you?',
    'awesome': 'That\'s awesome! 🎉 Glad you\'re enjoying Solana XP!',
    'cool': 'Right? Solana XP is pretty cool! 😎 What would you like to explore next?',
    'amazing': 'I know, right? Amazing! ⭐ Is there anything specific you\'d like to learn about?',
    
    'default': 'That\'s an interesting question! Solana XP features include a browser, mail client, system information, and more. Try asking about specific features, say hello, or type "help" for suggestions!'
  };
  ...
  if (recycleBinIcon && recycleBinWindow) {
